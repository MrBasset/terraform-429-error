...
TF_LOG=debug TF_LOG_PATH=./tf-v0.13.5.log terraform-13 plan -out plan.tfplan $FLAGS
if [ $? == 0 ]; then
  echo "Please use 'terraform apply -parallelism=1 \"plan.tfplan\"' to execute TF"
fi